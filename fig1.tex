...
\def \xmax {8cm}
\def \ymax {9cm}
\tikzstyle{arr}=[->, >=stealth, line width=1.2pt] % Style

% Labels
\tikzstyle{lab}=[font=\large]

\tikzstyle{life}=[line width=2pt]
\tikzstyle{connec}=[dashed]
\tikzstyle{connecc}=[dashed, line width=1.5, draw=red]
\tikzstyle{cntr}=[line width=1.5pt, draw=red]

% For each individual, we set the birth (tb) and death (td) times
% The individuals are labeled a, b, c,....
\def \dx {0.75}
\def \tba {0}
\def \tda {6}
\def \xa {\dx cm}
...
% Axes
\draw[arr] (-\daxis, 0)--(2*\xmax, 0); % Horizontal
\draw[arr] (0, -\daxis)--(0, \ymax); % Vertical
...
\node[plchd, draw=red] (ya) at (\xa, \tda){};
